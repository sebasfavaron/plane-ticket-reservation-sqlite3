...
int createFlight(int seatAmount, char *flightName){
    ...
    if(alreadyExists || rc != SQLITE_OK) {
        return -1;
    }

    for(int currSeat=0; currSeat<seatAmount; currSeat++){
        char sql[100];
        snprintf(sql, sizeof(sql), "insert into %s values(\"%s\",%d,%d)", TABLENAME, flightName, currSeat, isTaken);
        printf("Command: %s\n",sql);
        ...
        if(rc != SQLITE_OK){
            ...
            printf("Value %i inserted correctly into table %s\n\n", currSeat, TABLENAME);
        }
    }
    return 0;
}
...
int reserveSeat(char *flightName, int seatNumber){
    ...
    if(rc != SQLITE_OK){
        ...
        printf("Query correctly executed\n\n");
        return 0;
    }
}
...
int main(int argc, char **argv){
    //todo: falta crear airport.db si no existe y crear ahi la tabla flights con su clave
    ...
    createFlight(10,"ECO123");
    reserveSeat("ECO123", 2);
